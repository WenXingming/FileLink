...
public:
    ...
    // 查询上传会话
    bool get_session_progress(const std::string& ownerUserId, const std::string& uploadIdHex, uint64_t& out_offset, uint64_t& out_totalSize);
    bool get_session(const std::string& ownerUserId, const std::string& uploadIdHex, db::UploadSession& out_session);

    // 创建与写入上传会话
    bool create_session(const std::string& ownerUserId, uint64_t totalSize, const std::string& metadataHeader, const std::string& host, std::string& out_uploadIdHex);
    UploadChunkResult write_session_chunk(const std::string& ownerUserId, const std::string& uploadIdHex, uint64_t clientOffset, const std::string& chunkData, uint64_t& out_newOffset);
    UploadTerminationResult terminate_session(const std::string& ownerUserId, const std::string& uploadIdHex);
...
private:
    ...
    std::string update_stream_hash(const std::string& uploadIdHex, const std::string& partPath, uint64_t offset, const std::string& chunkData, bool is_complete);
    ...
    bool complete_newly_published_session(const std::string& uploadIdBinary, const std::string& realHashHex);
