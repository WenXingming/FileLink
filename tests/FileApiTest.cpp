...
TEST_F(FileApiTest, ListsOnlyCurrentUsersFiles) {
    ...
    ShareApiRouter share_api_router(server, share_service, object_store, auth_service);
    FileApiRouter router(server, file_service, object_store, auth_service, share_api_router);
...
TEST_F(FileApiTest, DownloadsOnlyOwnersFileThroughInternalRedirect) {
    ...
    FileService file_service(pool_);
    ShareService share_service(pool_);
    ShareApiRouter share_api_router(server, share_service, store, auth_service);
    FileApiRouter router(server, file_service, store, auth_service, share_api_router);
